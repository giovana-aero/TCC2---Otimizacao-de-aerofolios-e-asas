...
pop.sweep = 0; # enflechamento da asa (trapezoidal simples) [graus]
...
# Valores de entrada para asas trapezoiduplas:
# b    - envergadura
...
if pop.type == 0: # Se a asa for trapezoidal simples
    ...
    naca_r = pop.af_r
    naca_t = pop.af_t
    x_r,y_r = naca4(str(int(naca_r[0]))+str(int(naca_r[1]))+str(int(naca_r[-1])),dat.np-1)
    x_t,y_t = naca4(str(int(naca_t[0]))+str(int(naca_t[1]))+str(int(naca_t[-1])),dat.np-1)
    coo_r = np.hstack((np.array([x_r]).transpose(),np.array([y_r]).transpose()))
    coo_t = np.hstack((np.array([x_t]).transpose(),np.array([y_t]).transpose()))
    coo_r = coo_r[0:-1,:]*c_r
    coo_t = coo_t[0:-1,:]*c_t
    ...
    if 'L' in pop.af_m:
        ...
        naca_m = pop.af_m
        x_m,y_m = naca4(str(int(naca_m[0]))+str(int(naca_m[1]))+str(int(naca_m[-1])),dat.np-1)
        coo_m = np.hstack((np.array([x_m]).transpose(),np.array([y_m]).transpose()))
        coo_m = coo_m[0:-1,:]*c_m
