...
# Os aerofólios são lidos a partir de arquivos de coordenadas. Ao final é feita 
# uma simulação no prompt de comando, os resultados são lidos, e é apresentada 
# uma figura da asa.

# neste caso, a simulação é feita pelo método VLM
...
# // ===== Criar uma asa trapezoidal simples ou dupla ====//
...
cm = 2; # Corda do meio
ct = 1; # Corda da ponta
af_r = 'coordenadas.dat' # perfil da raiz
af_m = 'MH 24.dat' # perfil do meio
af_t = 'Wortmann FX 61-184.dat' # perfil da ponta
tw_m = -30 # Torção geométrica no meio
...
if plan_op == 1: # Asa trapezoidal simples
    ...
    vsp.ReadFileAirfoil( xsec_r, 'coordenadas.dat' )
    vsp.Update()
    ...
    b1 = S1*2/(cr + cm)
    b2 = S2*2/(cm + ct)
    lambda_le1 = math.degrees(math.asin((cr - cm)/b1))
    lambda_le2 = math.degrees(math.asin((cm - ct)/b2))
    ...
    # entre os 8 possíveis pra manipular a planta da asa
    vsp.SetDriverGroup( wid, 1, vsp.AREA_WSECT_DRIVER, vsp.ROOTC_WSECT_DRIVER, vsp.TIPC_WSECT_DRIVER )
    vsp.SetDriverGroup( wid, 2, vsp.AREA_WSECT_DRIVER, vsp.ROOTC_WSECT_DRIVER, vsp.TIPC_WSECT_DRIVER )
    ...
    vsp.ReadFileAirfoil( xsec_m, af_m )
    vsp.Update()
...
# vsp.ExportFile(name+'.csv',vsp.SET_ALL,vsp.DEGEN_GEOM_CSV_TYPE)
