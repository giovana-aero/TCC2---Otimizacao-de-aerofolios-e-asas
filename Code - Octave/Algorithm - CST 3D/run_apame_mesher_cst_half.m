function pop = run_apame_mesher_cst_half(pop,dat,op1,op2,fig1,fig2,text)
% Esta função gera a malha de asas pro apame. Asas são trapezoidais simples ou
% duplas. Esta função gera apenas metade da asa
...
if pop.type == 0 % Se a asa for trapezoidal simples
    ...
    sec_N = 2 + nb; % Número de seções transversais
...
%    NODE = [NODE;coo_t_R(:,1)+dx_t,zeros(sec_af_N,1)+b/2,coo_t_R(:,2)];
    ...
    sec_N = 3 + nb1 + nb2; % Número de seções transversais
    far = b*2;
...
%    NODE = [NODE;coo_t_R(:,1)+dx_t,zeros(sec_af_N,1)+b/2,coo_t_R(:,2)];
    ...
    S = S1 + S2;
    mac = ((S1/b1 + S2/b2)/2)/2; % obtido a partir das relações da razão de aspecto
