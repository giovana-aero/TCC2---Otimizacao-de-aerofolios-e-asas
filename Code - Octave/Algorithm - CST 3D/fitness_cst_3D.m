...
function pop = fitness_cst_3D(pop,dat,select2)
...
for P = 1:dat.cases % Fazer loops para cada condição de voo
    ...
    if dat.coeff_op(P,4) ~= '!' && dat.coeff_op(P,4) ~= 'c' && dat.coeff_op(P,4) ~= 'k'
        ...
        elseif dat.coeff_op(P,4) == 'q' % Alcançar um M específico
            ...
            if Mtgt == 0 && dat.aoa(P) == 0 % Se o CM alvo for igual a zero e o ângulo de ataque for nulo
                for i = [select2]
                    pop(i).score = 1 + pop(i).score;
                end
            elseif Mtgt == 0 % Se o M alvo for igual a zero
...
if dat.coeff_op(1,4) == 'c' || dat.coeff_op(1,4) == 'k' 
    for i = [select2]
        ...
        elseif dat.coeff_op(1,4) == 'k' % Se o valor for específico
