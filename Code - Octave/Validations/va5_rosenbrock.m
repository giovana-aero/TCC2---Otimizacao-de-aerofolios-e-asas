%% Validação 6 - Função de Rosenbrock
...
dat.t = 3;                               % Dimensão da função
dat.mu = 0.10;                            % Probabilidade de mutação
...
for i = 1:dat.n
    for j = 1:dat.t
        pop(i).gen(j) = rand*(5)*randi([-1 1]);
    end
end

condition = 0; convergence = 0;
%% loop principal
for loop = 1:dat.iter
    ...
    for i = 1:dat.n
        ...
        sum = 0;
        for j = 1:dat.t-1
            sum = (pop(i).gen(j)-1)^2 + 100*(pop(i).gen(j+1) - pop(i).gen(j)^2)^2 + sum;
        end
    ...
    sum = 0;
    for j = 1:dat.t-1
        sum = (pop(pos).gen(j)-1)^2 + 100*(pop(pos).gen(j+1) - pop(pos).gen(j)^2)^2 + sum;
    end
    f_tgt = sum;
    dat.best(loop) = f_tgt;
    ...
    for i = i:dat.n
        ...
        if randi([1 dat.mu_number]) == 1
            ...
            k = 1;
            for j = start:range
                children(i).gen(j) = rand*(5)*randi([-1 1]);
                k = k + 1;
            end
