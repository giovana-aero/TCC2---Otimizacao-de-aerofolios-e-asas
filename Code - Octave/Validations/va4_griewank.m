%% Validação 5 - Função de Griewank
...
for i = 1:dat.n
    for j = 1:dat.t
        pop(i).gen(j) = rand*(600)*randi([-1 1]);
    end
end
...
for loop = 1:dat.iter
    ...
    for i = 1:dat.n
        ...
        for j = 1:dat.t
            ...
            pro = cos(pop(i).gen(j)/sqrt(j))*pro;
        end
        
        f_tgt = 4000^-1*sum - pro + 1;
    ...
    for j = 1:dat.t
        ...
        pro = cos(pop(pos).gen(j)/sqrt(j))*pro;
    end
    f_tgt = 4000^-1*sum - pro + 1;
    dat.best(loop) = f_tgt;
    ...
    for i = i:dat.n
        ...
        if randi([1 dat.mu_number]) == 1
            ...
            k = 1;
            for j = start:range
                children(i).gen(j) = rand*(2*pi)*randi([-1 1]);
                k = k + 1;
            end
