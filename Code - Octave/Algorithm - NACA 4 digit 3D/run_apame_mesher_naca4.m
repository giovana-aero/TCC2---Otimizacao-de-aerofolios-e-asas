function pop = run_apame_mesher_naca4(pop,dat,op1,op2,fig1,fig2,text)
...
% af_t - nome do aerofólio da ponta [m,p,t]
% nb   - número de seções intermediárias (raiz/ponta) 
% nb1  - número de seções intermediárias (raiz/meio) (permite opção 'L')
% nb2  - número de seções intermediárias (meio/ponta)
...
if pop.type == 0 % Se a asa for trapezoidal simples
    ...
    coo_r = fourdigit(pop.af_r,dat.np,dat.np_op); coo_r = coo_r(1:end-1,:)*c_r;
    coo_t = fourdigit(pop.af_t,dat.np,dat.np_op); coo_t = coo_t(1:end-1,:)*c_t;
    ...
    if ismember('L',pop.af_m)
        coo_m = airfoil_interpolation(coo_r,coo_t,b1/b,0,tw_t*b1/b);
    else
        coo_m = fourdigit(pop.af_m,dat.np,dat.np_op);
...
if op2 == 1 % TraÃ§ar os formatos originais
...
elseif op2 == 2 % TraÃ§ar considerando as rotações
    figure(fig2),hold on,grid on,axis equal
    plot(coo_r(:,1),coo_r(:,2),'k')
    if pop.type == 1
        coo_m_dx = coo_m;
