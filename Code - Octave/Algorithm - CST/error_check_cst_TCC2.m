...
if dat.N1 ~= 0.5 || dat.N2 ~= 1
    warning('Coeficientes N1 e N2 da função class estão especificando uma geometria que não é um aerofólio'),pause(5)
end
...
if ~isfield(dat,'or_v_ex') % Válido apenas pro algoritmo normal
...
if isfield(dat,'or_v_ex') % Válida apenas pro algoritmo específico
	if dat.or_v_ex(dat.BPn+1) + dat.B_ext2(2) < dat.B_ext2(1)
        error('Extensão de Beta 2 insuficiente para cumprir o requisito de separação')
    end
end

if isfield(dat,'or_v_ex') % Válida apenas pro algoritmo específico
	if dat.le_R_ext1(3) <= 0 || dat.le_R_ext2(3) <= 0
        error('Valor mínimo do raio do bordo de ataque deve ser maior que zero')
    end
end

if isfield(dat,'or_v_ex') % Válida apenas pro algoritmo específico
    if length(dat.or_v_ex) ~= length(dat.or_v_in)
        error('Vetores originais de extradorso e intradorso devem ter comprimento igual (mesmo grau do polinômio)')
    end
end

if dat.BPn <= 1
    error('O polinômio de Bernstein deve ter grau de no mínimo 2')
end
