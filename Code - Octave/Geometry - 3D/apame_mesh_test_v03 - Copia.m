...
% A continuar depois: consertar as torções de asa. Temos duas opções possíveis:
% - torções independentes: torção da ponta e do meio são valores determinados pelo usuário;
% - torções lineares: a torção do meio é calculada a partir da torção da ponta
...
tw_m = -10; % Torção geométrica no meio
tw_t = -10; % Torção geométrica na ponta
...
b2 = b - b1;
nb = 2;    % Número de seções intermediárias entre raiz/meio e meio/ponta  (considerando apenas um lado da asa)
...
% ALguns dados a mais
sec_af_N = size(coo_r,1); % Número de nós por seção (aerofólio)
sec_N = 5 + nb*4; % Número de seções transversais
...
if nb > 0
    ...
    wing_sec1 = repmat(wing_sec1,nb,1); % Inicializar
    ...
    for i = 1:length(op_vec)
        ...
        tw_sec = op_vec(i)*tw_t;%*cosd(c_t*op_vec(i)+c_r*(1-op_vec(i)));
        wing_sec1(i).coo = airfoil_interpolation(coo_m,coo_t,op_vec(i),-op_vec(i)*((b-b1)/2)-b1/2,tw_sec);
        % Aplicar o translado
        wing_sec1(i).dx = op_vec(i)*dx_t2 + dx_m; 
        wing_sec1(i).coo(:,1) = wing_sec1(i).coo(:,1) + wing_sec1(i).dx;
        % Adicionar ao lado esquerdo da asa
        NODE(sec_af_N*i+1:sec_af_N*(i+1),:) = wing_sec1(i).coo;
    end
...
if nb > 0
    ...
    wing_sec2 = repmat(wing_sec2,nb,1); % Inicializar
    op_vec = linspace(1,0,2+nb); op_vec = op_vec(2:end-1); % Definição do formato da interpolação em função dos originais
    ...
    for i = 1:length(op_vec)
        % Fazer interpolação e aplicar torção geométrica
        tw_sec = op_vec(i)*tw_t;%*cosd(c_t*op_vec(i)+c_r*(1-op_vec(i)));
        wing_sec2(i).coo = airfoil_interpolation(coo_r,coo_m,op_vec(i),-op_vec(i)*(b1/2),tw_sec);
        % Aplicar o translado
        wing_sec2(i).dx = op_vec(i)*dx_m; % op_vec(i)*(b/2)*dx_t/(b/2)
        wing_sec2(i).coo(:,1) = wing_sec2(i).coo(:,1) + wing_sec2(i).dx;
        % Adicionar ao lado esquerdo da asa
        temp(sec_af_N*(i-1)+1:sec_af_N*i,:) = wing_sec2(i).coo;
    end
