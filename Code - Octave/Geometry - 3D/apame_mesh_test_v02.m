...
nb = 8;    % Número de seções intermediárias entre a raiz e a ponta (considerando apenas um lado da asa)
...
if nb > 0
    ...
    for i = 1:length(op_vec)
        ...
        % Aplicar o translado
        wing_sec(i).dx = op_vec(i)*dx_t; % op_vec(i)*(b/2)*dx_t/(b/2)
...
fid = fopen('wings_rewrite_v02.inp','w');
...
if sim_op == 1
    ...
    system('apame_win64.exe < apame_input_v02.txt');clc

    if exist('wings_rewrite_v02.log','file')
    
        result_ID = fopen('wings_rewrite_v02.log','r');
        result = fscanf(result_ID,'%c');
        disp(result)
        fclose(result_ID);
        delete('fort.2');
        delete('wings_rewrite_v02.log');
        delete('wings_rewrite_v02.res');
